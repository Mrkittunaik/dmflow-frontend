...
const DMFLOW_CONFIG = {
 BACKEND_URL: (document.querySelector('meta[name="dmflow-api"]')?.content || window.DMFLOW_API_URL || '').replace(/\/$/, ''),
};

// ─── Storage helpers ──────────────────────────────────────
function dmSetUser(u) {
  // Only store display fields — never email, plan, or Instagram token details
  try {
    ...
    const safe = {
      ...
      plan:       u.plan || 'free',
      dmsSent:    u.dmsSent  || 0,
      dmsLimit:   u.dmsLimit || 500,
      igConnected: ig.connected || false,
      // Full instagram display object so profilePic + username work everywhere
      instagram: {
    ...
    localStorage.setItem('dmflow_user', JSON.stringify(safe));
  } catch(e) {}
}
...
function googleLogin(e) {
  ...
  window.location.href = DMFLOW_CONFIG.BACKEND_URL + '/app/auth/google';
}
...
async function connectInstagram(username) {
  ...
  try {
    ...
    const res = await fetch(DMFLOW_CONFIG.BACKEND_URL + '/app/auth/instagram/url', {
...
async function doRegister() {
  ...
  try {
    const res  = await fetch(DMFLOW_CONFIG.BACKEND_URL + '/app/auth/register', {
      method: 'POST',
    ...
    dmSetUser(data.user);
    window.location.href = '../onboarding/platform-select.html';
  } catch(err) {
...
function sendOtp() {
  showToast('Phone registration is not yet configured. Please use email sign-up.','error');
}
...
function verifyOtp() {
  ...
  // Phone OTP requires backend SMS integration (e.g. Twilio/MSG91) — not yet configured
  showToast('Phone login is not yet configured. Please use email login.','error');
  hideEl('step2otp'); showEl('step2email');
}
...
async function doLogin() {
  ...
  try {
    const res  = await fetch(DMFLOW_CONFIG.BACKEND_URL + '/app/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    localStorage.setItem('dmflow_token', data.token);
    dmSetUser(data.user);
    window.location.href = data.user.instagram?.connected ? '/app/platforms/instagram/dashboard.html' : '/app/onboarding/platform-select.html';
  } catch(err) {
...
function verifyLoginOtp() {
  ...
  const otp   = Array.from(boxes).map(b => b.value).join('');
  if (otp.length < 6) { showToast('Enter all 6 digits.','error'); return; }
  // Phone OTP requires backend SMS integration — not yet configured
  showToast('Phone login is not yet configured. Please use email login.','error');
  hideEl('step3otp'); showEl('step2login');
}
...
async function sendResetLink() {
  ...
  try {
    ...
    await fetch(DMFLOW_CONFIG.BACKEND_URL + '/app/auth/forgot-password', {
...
function signOut(e) {
  ...
  try {
    window.top.location.href = '/app/auth/login.html';
  } catch(err) {
