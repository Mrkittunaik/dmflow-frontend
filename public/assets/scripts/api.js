...
const API = (() => {
  ...
  function _handleUnauth(silent) {
    ...
    if (!silent) {
      try {
        ...
        top.location.replace('/app/auth/login.html');
      } catch(e) {
  ...
  return {
    ...
    // ── Auth ────────────────────────────────────────────────────
    register:       (name, email, password) => req('POST', '/app/auth/register', { name, email, password }),
    login:          (email, password)       => req('POST', '/app/auth/login',    { email, password }),
    forgotPassword: (email)                 => req('POST', '/app/auth/forgot-password', { email }),
    resetPassword:  (token, password)       => req('POST', '/app/auth/reset-password', { token, password }),
    logout:         ()                      => req('POST', '/app/auth/logout', null, { silent: true }),

    // Google OAuth
    googleLoginUrl: () => BASE_URL + '/app/auth/google',

    // Instagram OAuth
    instagramConnectUrl: async () => {
      const token = getToken();
      const res   = await fetch(BASE_URL + '/app/auth/instagram/url', {
    ...
    resubscribeWebhook: () => {
      return req('POST', '/app/auth/instagram/resubscribe');
    },
