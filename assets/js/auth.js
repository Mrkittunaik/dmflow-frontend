...
async function doLogin() {
  ...
  try {
    ...
    localStorage.setItem('dmflow_token', data.token);
    dmSetUser(data.user);
    window.location.href = data.user.instagram?.connected ? '../../app.html' : '../onboarding/platform-select.html';
  } catch(err) {
...
function signOut(e) {
  if (e) e.preventDefault();
  dmClearUser();
  // Use window.top to escape iframes; calculate path from top frame
  const depth = (window.top.location.pathname.match(/\//g) || []).length - 1;
