...
const API = (() => {
  // ── Config: reads from meta tag only — no hardcoded fallback ──
  const metaUrl = document.querySelector('meta[name="dmflow-api"]')?.content;
  const BASE_URL = metaUrl || window.DMFLOW_API_URL || '';
  ...
  return {
    ...
    // saveInstagram removed — token is saved server-side in /auth/instagram/callback (never touches the browser)
    ...
    refreshIgToken: ()            => req('POST', '/api/ig/refresh-token'),
