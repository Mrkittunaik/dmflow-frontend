...
const API = (() => {
  ...
  return {
    ...
    // ── WhatsApp OAuth (Meta Embedded Signup) ──────────────────────
    whatsappConnectUrl: async () => {
      const token = getToken();
      const res   = await fetch(BASE_URL + '/auth/whatsapp/url', {
        headers: token ? { Authorization: 'Bearer ' + token } : {}
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || 'Could not get WhatsApp connection config.');
      }
      return res.json(); // { appId, configId, state }
    },
    connectWhatsApp: (code, wabaId, phoneNumberId) => {
    ...
    resubscribeWhatsAppWebhook: (id) => req('POST', '/api/whatsapp/connections/' + id + '/resubscribe'),
    checkWhatsAppHealth:    (id)    => req('POST', '/api/whatsapp/connections/' + id + '/health', null, { noCache: true }),
    registerWhatsAppNumber: (id, pin) => req('POST', '/api/whatsapp/connections/' + id + '/register', { pin }),

    // ── WhatsApp Inbox ──────────────────────────────────────────────
    getWhatsAppInbox:        (page, search) => req('GET', '/api/whatsapp/inbox?page=' + (page || 1) + (search ? '&search=' + encodeURIComponent(search) : ''), null, { noCache: true }),
    getWhatsAppConversation: (id)           => req('GET', '/api/whatsapp/inbox/' + id, null, { noCache: true }),
    sendWhatsAppReply:       (id, text)     => { _cacheInvalidate('/api/whatsapp/inbox'); return req('POST', '/api/whatsapp/inbox/' + id + '/reply', { text }); },
    markWhatsAppRead:        (id)           => req('PATCH', '/api/whatsapp/inbox/' + id + '/read', null, { noCache: true }),
    assignWhatsAppConversation: (id, agentId) => req('PATCH', '/api/whatsapp/inbox/' + id + '/assign', { agentId }),
    addWhatsAppNote:         (id, note)     => req('POST', '/api/whatsapp/inbox/' + id + '/note', { note }),
